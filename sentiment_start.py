...
class ExRNN(nn.Module):
    ...
    def forward(self, x, hidden_state):
        ...
        # Compute new hidden state with activation (standard Elman RNN)
        hidden = self.sigmoid(self.in2hidden(combined))
        # Compute output from hidden state
        output = self.sigmoid(self.hidden2output(hidden))
...
class ExGRU(nn.Module):
    ...
    def forward(self, x, hidden_state):
        ...
        h_t = torch.tanh(self.candidate(combined_reset))

        hidden = (1-z_t) * hidden_state + z_t * h_t
        output = torch.sigmoid(self.hidden2output(hidden))
        return output, hidden
